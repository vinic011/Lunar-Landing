...
class DQNAgent:
    ...
    def __init__(self, state_size, action_size, gamma=0.95, epsilon=0.25, epsilon_min=1.0, epsilon_decay=0.995, learning_rate=0.001, buffer_size=4098):
        """
    ...
    def make_model(self):
        ...
        model.add(layers.Dense(units=150, activation=activations.relu, input_shape=(self.state_size,)))
        model.add(layers.Dense(units=150, activation=activations.relu))
    ...
    def act(self, state):
        ...
        greedy_action = np.argmax(self.model.predict(state))
